...
{
    public sealed class TypeTitle : AggregateRoot<int>
    {
        public string Name { get; private set; } = null!;

        private TypeTitle() { }
        private TypeTitle(string name) { Name = name; }

        public static TypeTitle Create(string name)
        {
            GuardException.Against.Null(name, nameof(name), "Вы не заполнил поле!");

            return new TypeTitle(name);
        }
