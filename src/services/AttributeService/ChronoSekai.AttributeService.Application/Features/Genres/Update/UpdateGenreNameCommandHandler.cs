...
{
    public sealed class UpdateGenreNameCommandHandler
        (
            IApplicationDbContext context,
            IValidationService validationService
        ) : IRequestHandler<UpdateGenreNameCommand, Result>
    {
        private readonly IApplicationDbContext _context = context;
        private readonly IValidationService _validationService = validationService;

        public async Task<Result> Handle(UpdateGenreNameCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var entity = await _context.Genres.FirstOrDefaultAsync(x => x.Id == request.Id);
