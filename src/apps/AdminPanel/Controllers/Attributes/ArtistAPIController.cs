...
namespace AdminPanel.Controllers.Attributes
{
    ...
    public class ArtistAPIController(ArtistAPIClient artistAPIClient, ILogger<ArtistAPIController> logger) : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult<ArtistDTO>> CreateAsync([FromBody] CreateArtistRequest request)
        {
            try
            {
                var result = await _artistAPIClient.CreateAsync(request);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(int id, [FromBody] UpdateArtistNameRequest request)
        {
            ...
            try
            {
                await _artistAPIClient.UpdateNameAsync(id, request);
                return NoContent();
            }
            catch (HttpRequestException ex)
            {
        ...
        public async Task<IActionResult> DeleteAsync(int id)
        {
            try
            {
                await _artistAPIClient.DeleteAsync(id);
