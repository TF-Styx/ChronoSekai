...
namespace AdminPanel.Controllers.Attributes
{
    ...
    public class PublisherAPIController(PublisherAPIClient publisherAPIClient, ILogger<PublisherAPIController> logger) : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult<PublisherDTO>> CreateAsync([FromBody] CreatePublisherRequest request)
        {
            try
            {
                var result = await _publisherAPIClient.CreateAsync(request);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(int id, [FromBody] UpdatePublisherNameRequest request)
        {
            ...
            try
            {
                await _publisherAPIClient.UpdateNameAsync(id, request);
                return NoContent();
            }
            catch (HttpRequestException ex)
            {
        ...
        public async Task<IActionResult> DeleteAsync(int id)
        {
            try
            {
                await _publisherAPIClient.DeleteAsync(id);
