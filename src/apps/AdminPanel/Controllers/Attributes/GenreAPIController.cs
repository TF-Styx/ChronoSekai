...
namespace AdminPanel.Controllers.Attributes
{
    ...
    public class GenreAPIController(GenreAPIClient genreAPIClient, ILogger<GenreAPIController> logger) : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult<GenreDTO>> CreateAsync([FromBody] CreateGenreRequest request)
        {
            try
            {
                var result = await _genreAPIClient.CreateAsync(request);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateAsync(int id, [FromBody] UpdateGenreNameRequest request)
        {
            ...
            try
            {
                await _genreAPIClient.UpdateNameAsync(id, request);
                return NoContent();
            }
            catch (HttpRequestException ex)
            {
        ...
        public async Task<IActionResult> DeleteAsync(int id)
        {
            try
            {
                await _genreAPIClient.DeleteAsync(id);
